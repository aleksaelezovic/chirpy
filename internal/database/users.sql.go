...
const changeChirpyRedStatus = `-- name: ChangeChirpyRedStatus :one
UPDATE users
SET is_chirpy_red = $2, updated_at = NOW()
WHERE id = $1 RETURNING id, created_at, updated_at, email, hashed_password, is_chirpy_red
`

type ChangeChirpyRedStatusParams struct {
	ID          uuid.UUID `json:"id"`
	IsChirpyRed bool      `json:"is_chirpy_red"`
}

func (q *Queries) ChangeChirpyRedStatus(ctx context.Context, arg ChangeChirpyRedStatusParams) (User, error) {
	row := q.db.QueryRowContext(ctx, changeChirpyRedStatus, arg.ID, arg.IsChirpyRed)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
